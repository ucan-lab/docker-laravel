...
class ItemizedOrderPolicy
{
    ...
    public function create(User $user, Store $store, Bill $bill, int $requestBillId): bool
    {
        ...
        if ($this->roleRepo->hasPermission($groupRole->role, RouteActionTarget::ROUTE_ACTION_TARGETS['EXTENSIONSET_STORE_TARGET_GROUP']['id'])) {
            return true;
        }
        ...
        foreach ($storeRoles as $storeRole) {
            if ($this->roleRepo->hasPermission($storeRole->role, RouteActionTarget::ROUTE_ACTION_TARGETS['EXTENSIONSET_STORE_TARGET_STORE']['id'])) {
                return true;
            }
        }
